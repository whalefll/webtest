...
var baidushare = "0";
//是否开启百度分享，0关闭，启用本地分享填写/statics/api/js/share.js，启用官网分享填写/bdimg.share.baidu.com/static/api/js/share.js（不支持https）
		...
		'copylink': function(){
			if(copyshort==1){
				if($(".copylink").length){
					$.ajax({ 
						type : 'GET',  
				        url : '/api.weibo.com/2/short_url/shorten.json?source=2849184197&url_long='+encodeURIComponent(stui.browser.url),
				        dataType : 'JSONP',   
				        success : function(r) {
...
console.log("\n %c WCCMS 3.5\u6F14\u793A %c by \u5fd8\u5ddd\u5f71\u89c6 | WCCMS 3.5 \n","color:#fff;background:#71bef1;padding:5px 0;", "color:#444;background:#eee;padding:5px 0;");
